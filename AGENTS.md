...
* All test data should be in `/testdata` if it is required for tests. Otherwise, test generated data must be git ignored and not added to the repo. When generating test output data, use a git-ignored name or simply generate it outside the project directory (e.g., `/tmp/`).
