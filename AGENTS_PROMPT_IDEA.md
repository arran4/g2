...
Use `mvdan.cc/sh/v3/syntax` to parse `make.conf` files into an AST. Note that `repos.conf` uses an INI-style format, so you will need to use a separate parsing strategy for it (such as a custom AST-preserving INI parser or a specific INI library). Implement a `ConfigurationModel` struct that holds the root AST nodes and can map variables (like `COMMON_FLAGS`) back to their specific `*syntax.Assign` nodes and their origin file paths. Do not implement a UI yet, focus purely on the AST models, parsing logic (handling directories), and the logic required to accurately write an altered AST back to its original file."
