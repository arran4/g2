...
func TestRepoLayoutLintRule(t *testing.T) {
	...
	_ = os.MkdirAll(filepath.Join(tempDir, "metadata"), 0755)
	_ = os.MkdirAll(filepath.Join(tempDir, "eclass"), 0755)

	// Create categories file
	_ = os.WriteFile(filepath.Join(tempDir, "profiles", "categories"), []byte("app-test\n"), 0644)
	_ = os.MkdirAll(filepath.Join(tempDir, "app-test"), 0755)

	// Create stray directory and file
	_ = os.MkdirAll(filepath.Join(tempDir, "stray-dir"), 0755)
	_ = os.WriteFile(filepath.Join(tempDir, "stray-file.txt"), []byte("test"), 0644)

	// Create a .g2ignore file
	_ = os.WriteFile(filepath.Join(tempDir, ".g2ignore"), []byte("stray-dir\nignored-file.txt\n"), 0644)
	_ = os.WriteFile(filepath.Join(tempDir, "ignored-file.txt"), []byte("test"), 0644)
